...
el diseño de un dispositivo trepador autónomo capaz de ascender
...
proceso de diseño, las restricciones del sistema y los criterios de selección para los prototipos.
\end{abstract}
...
a este estudio, se realizará posteriormente una descomposición funcional del sistema y se propondrá un concepto de diseño por cada principio de locomoción identificado. Estos conceptos serán evaluados en una matriz de decisión técnica para seleccionar los dos más prometedores,
los cuales serán desarrollados a profundidad en la siguiente etapa.
...
del mecanismo. El análisis comparativo de las tres familias identificadas, siendo estas la
\textbf{rodadura sincronizada}, el \textbf{avance alternante} y la \textbf{locomoción por
...
\begin{enumerate}
    ...
    \item Identificar los principios de locomoción vertical documentados en
    ...
    \item Aplicar una metodología estructurada de descomposición funcional al
    ...
    \item Evaluar los conceptos propuestos mediante una matriz de decisión
    ...
    \item Desarrollar el diseño detallado de los dos conceptos seleccionados,
    ...
    \item Construir y validar experimentalmente el prototipo final
    ...
    \item Documentar de forma continua y rigurosa todo el proceso de diseño
...
tensión (basado en el diseño de la Figura \ref{fig:bastidor_abierto})}
...
\emph{generar fuerza normal} (contacto con las ruedas/rodillos), \emph{transmitir movimiento}
...
es la de mayor complejidad en este concepto, al requerir un lazo de control para lograr la
sincronización.

\subsubsection{Concepto 2 -- Escalador alternante por módulos de agarre (basado en el diseño de la Figura \ref{fig:husillo_katz})}
...
\subsubsection{Concepto 3 -- Escalador por palanca con resortes de tensión (basado en el diseño de la Figura \ref{fig:resorte_sadeghi})}
...
durante el descenso controlado al invertir el sentido de giro del actuador.
...
optimista \cite{KaiserEtAl2024ElastomerFriction}. En \emph{fabricabilidad}, se castigan los conceptos que requieren muchas piezas especiales o tolerancias difíciles de sostener en un prototipo \cite{UlrichEppinger2012}.
...
penalizan los conceptos cuya elaboración requiere procesos especializados o tiempos de
...
Los tres conceptos se evalúan con base en ocho criterios derivados del estado del
arte y del enunciado del proyecto. Los pesos de cada criterio fueron asignados en
proporción directa a dos fuentes: la relevancia que el enunciado del curso otorga a
cada requerimiento, y los factores de falla identificados en la literatura revisada.
Así, \emph{retorno seguro} recibe el mayor peso (18\%) por ser el segundo rubro de
mayor puntaje en la demostración final del curso (20~puntos sobre 50). \emph{Cumplimiento
de no daño} recibe 15\% por ser el requerimiento más enfatizado en el enunciado
(\emph{``Por sobre todo, no causar daño al tronco''}). \emph{Adaptación al diámetro},
\emph{margen anti-deslizamiento} y \emph{complejidad y fabricabilidad} reciben 13\%
cada uno por corresponder a los factores de falla más frecuentes documentados en la
literatura de robots trepadores \cite{GuiTangMukhopadhyay2018, Amaran2021,
KaiserEtAl2024ElastomerFriction}. \emph{Viabilidad de construcción} y
\emph{estabilidad global} reciben 10\% cada uno, y \emph{complejidad de control}
recibe 8\%, reflejando su importancia secundaria pero no despreciable para el éxito
del prototipo \cite{UlrichEppinger2012}. La escala de puntuación es de 1 a 5, donde
1 indica la puntuación mínima y 5 el máximo de puntos posibles respecto al criterio
evaluado. El puntaje ponderado de cada celda se obtiene como $\text{WP} = P \times w$.
...
Fabricabilidad               & 13\% & 3 & 0.39 & 2 & 0.26 & 4 & 0.52 \\
...
de los dos conceptos seleccionados, desglosados por subsistema funcional para facilitar
la planificación de adquisición de materiales y la asignación de tareas en la etapa de
diseño detallado.

\begin{table}[H]
\centering
\caption{Recursos específicos -- Concepto~3 (palanca con resortes de tensión).}
\label{tab:recursos_c3}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.18\linewidth} p{0.16\linewidth} p{0.55\linewidth}}
\toprule
\textbf{Subsistema} & \textbf{Tipo} & \textbf{Recurso} \\
\midrule
Chasis
    & Material
    & Madera MDF 9\,mm (primer prototipo); perfiles aluminio
      20$\times$20\,mm para iteraciones posteriores \\
Chasis
    & Fabricación
    & Cortadora láser (MDF), impresora FDM -- PLA para
      soportes y conectores secundarios \\
Chasis
    & Software
    & SolidWorks (CAD), SimulationXpress (análisis estático) \\
\midrule
Sujeción pasiva
    & Material
    & Resortes de tensión ($k$ a determinar en memoria de
      cálculos), rodamientos 608ZZ,
      rodillos de goma \SI{70}{\milli\meter} \\
Sujeción pasiva
    & Herramientas
...
Tracción
    & Material
    & Motor NEMA~17 (12\,V, torque $\geq$\,0.5\,Nm),
      rueda motriz de goma \SI{70}{\milli\meter},
      acople de eje rígido 5\,mm \\
Tracción
    & Electrónica
    & Driver TMC2209, microcontrolador ESP32,
      batería LiPo 3S 2200\,mAh, cableado general \\
Tracción
    & Software
...
\begin{table}[H]
\centering
\caption{Recursos específicos -- Concepto~1 (rodillos sincronizados con resortes).}
...
Bastidor
    & Fabricación
    & Cortadora láser (MDF), impresora FDM -- PLA para
      conectores, articulaciones y soportes \\
Bastidor
    & Software
    & SolidWorks (CAD), SimulationXpress (análisis estático) \\
\midrule
Sujeción pasiva
    & Material
    & Resortes de tensión ($k$ a determinar),
      rodamientos 608ZZ,
...
Tracción
    & Material
    & 3 motores DC con encoder (12\,V),
      ruedas de goma \SI{65}{\milli\meter},
      acoples de eje rígidos 5\,mm \\
Tracción
    & Electrónica
...
Tracción
    & Software
    & PlatformIO (firmware), MATLAB (sincronización) \\
\midrule
Validación
    & Herramientas
    & Cinta métrica, cronómetro, báscula digital, cámara \\
